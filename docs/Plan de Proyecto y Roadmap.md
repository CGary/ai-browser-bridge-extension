...
* **Hito 6 (M6): Target-based Library Routing.** ~~Capacidad de enviar comandos a bibliotecas o contextos específicos de la IA de forma dirigida, extendiendo el contrato IPC, el Content Script y la lógica de enrutamiento.~~ — ✅ **COMPLETADO**. Incluye el campo `Target` en `ipc.Request` (`internal/ipc/ipc.go`), el `tabRegistry` con atributo `target`, `findTargetedTab()` y el error `target_not_found` en `background.js`. *(t22, t23, t24)*
* **Hito 7 (M7): Sistema de Calibración Dinámica de Selectores.** ~~Permitir ajustar en caliente los selectores DOM usados por el Content Script sin recompilar ni resideloadar la extensión, con persistencia local y difusión a todos los tabs activos.~~ — ✅ **COMPLETADO**. Comandos `probe-selectors`, `get-active-selectors`, `calibrate` y `reset-selectors`, con persistencia en `chrome.storage.local` (`aibbe_calibrations`) y broadcast vía `UPDATE_SELECTORS`. *(mission `dynamic-selector-calibration-system`)*
* **Hito 8 (M8): Infraestructura Docker + VPN.** ~~Aislar la ejecución del navegador Chrome en un contenedor dedicado, enrutando su tráfico a través de una VPN (ProtonVPN) para desacoplar la sesión del entorno anfitrión.~~ — ✅ **COMPLETADO**. `configs/docker/docker-compose.yml` orquesta un servicio `vpn` (`qmcgaw/gluetun`, ProtonVPN) y un servicio `chrome` (`linuxserver/chrome`) que comparte su red (`network_mode: "service:vpn"`). *(mission `dockerized-aibbe-stack-isolated-chrome`)*
