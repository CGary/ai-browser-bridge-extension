...
* **Daemon (Host de Native Messaging):** Binario residente desarrollado en Go. Mantiene el estado de la conexión persistente con el navegador a través de los flujos estándar (`stdin`/`stdout`). Opera como un servidor IPC (Inter-Process Communication) sobre un socket Unix (ej. `/tmp/aibbe.sock`), reenviando cada petición síncrona de la CLI hacia la extensión de forma estrictamente secuencial, sin procesamiento concurrente de múltiples solicitudes.
...
* **Extensión de Chromium:**
    * *Background Script / Service Worker:* Escucha los eventos provenientes del daemon, gestiona el enrutamiento bidireccional de los mensajes hacia pestañas de NotebookLM y controla el ciclo de vida de la inyección de scripts. Mantiene un `tabRegistry` (`Map<tabId, {state, service, target, lastSeen}>`) poblado vía HANDSHAKE que emite cada Content Script al registrarse (con `service="notebooklm"` y su propio `target`). Cuando la petición IPC incluye un `target`, `findTargetedTab(target)` localiza el tab libre correspondiente a ese contexto; si no se especifica `target`, se usa el primer tab libre disponible. El enrutamiento falla explícitamente con `no_free_tabs` (sin tabs libres) o `target_not_found` (el `target` solicitado no tiene un tab registrado y libre).
...
La arquitectura opera bajo un paradigma de almacenamiento volátil para el **payload transaccional**: el contexto técnico inyectado, las plantillas de prompts y el código resultante residen de manera exclusiva en la memoria operativa (RAM) durante la ejecución de cada comando, sin persistencia en el sistema de archivos local. Esto garantiza la latencia mínima y satisface el alcance del MVP para el flujo de generación.

Esta política **no aplica** a la configuración de calibración de selectores: `background.js` persiste deliberadamente los selectores calibrados en `chrome.storage.local` bajo la clave `aibbe_calibrations`, para que sobrevivan a reinicios del Service Worker y del navegador. Los comandos `calibrate`, `reset-selectors` y `get-active-selectors` leen y escriben este estado persistente.
...
* **Contrato de Datos (CLI - Daemon):** La transferencia de instrucciones a través del socket Unix emplea el mismo esquema JSON (`ipc.Request{Cmd, Target, Payload}`, ej. `{"cmd": "...", "target": "...", "payload": "..."}`), asegurando la interoperabilidad de las estructuras de datos y facilitando la depuración directa del flujo interno sin la sobrecarga de formatos de serialización binaria en esta iteración. El campo `target` permite el enrutamiento dirigido a contextos específicos.
* **Comandos soportados (`cmd`):**
    * `generate` — inyecta el prompt/contexto en el tab objetivo (o el primer tab libre) y extrae la respuesta generada.
    * `probe-selectors` — verifica en el DOM activo cuáles selectores calibrados o por defecto resuelven correctamente.
    * `get-active-selectors` — devuelve el conjunto de selectores actualmente en uso por el Content Script.
    * `calibrate` — persiste overrides de selectores en `chrome.storage.local` (clave `aibbe_calibrations`) y notifica a todos los tabs registrados vía `UPDATE_SELECTORS`.
    * `reset-selectors` — elimina la calibración persistida y restaura los selectores por defecto en todos los tabs.
    * Claves válidas de calibración (`VALID_SELECTOR_KEYS`): `INPUT`, `SUBMIT_BUTTON`, `RESPONSE_CONTAINER`, `RESPONSE_TEXT`, `THINKING_MARKERS`, `RESPONSE_READY_MARKERS`, `CITATION_NOISE`.
